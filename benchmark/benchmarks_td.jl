...
SUITE["GaussianDStatMMD"]["Medium_Float64"] = @benchmarkable compute_distances($x_medium_64, $alg)
SUITE["GaussianDStatMMD"]["Medium_Float32"] = @benchmarkable compute_distances($x_medium_32, $alg)

alg = GaussianDStatMMD(sqrt(d_small / 2.0))
SUITE["GaussianDStatMMD"]["Small_Float64"] = @benchmarkable compute_distances($x_small_64, $alg)
SUITE["GaussianDStatMMD"]["Small_Float32"] = @benchmarkable compute_distances($x_small_32, $alg)


# --------- KernelDStatMMD ---------
gaussian_kernel(bandwidth) = ExponentialKernel(; metric=SqEuclidean()) ∘ ScaleTransform(1 / bandwidth)
...
SUITE["KernelDStatMMD"]["Medium_Float64"] = @benchmarkable compute_distances($x_medium_64, $alg)
SUITE["KernelDStatMMD"]["Medium_Float32"] = @benchmarkable compute_distances($x_medium_32, $alg)

alg = KernelDStatMMD(gaussian_kernel(sqrt(d_small / 2.0)))
SUITE["KernelDStatMMD"]["Small_Float64"] = @benchmarkable compute_distances($x_small_64, $alg)
SUITE["KernelDStatMMD"]["Small_Float32"] = @benchmarkable compute_distances($x_small_32, $alg)
